...
public static class Report
{
                             ...
                             string version, IReadOnlyList<string> performed)
    {
        ...
        {
            Line("");
            if (ffu.AutoloaderActive)
            {
                Line("  !!! OSTRAAUTOLOADER ACTIVE - Ostrasort is read-only on this install !!!", ConsoleColor.Red);
                foreach (var e in ffu.Evidence) Line($"      - {e}", ConsoleColor.Red);
            }
            else
            {
                Line($"FFU DETECTED - {ffu.Summary} is supported; FFU ordering rules applied", ConsoleColor.Cyan);
                foreach (var e in ffu.Evidence) Line($"  - {e}", ConsoleColor.DarkGray);
            }
            foreach (var n in FfuAnalysis.Notices(a))
                Line($"  {n}", ffu.AutoloaderActive ? ConsoleColor.Yellow : (ConsoleColor?)null);
        }
        ...
        Line($"WARNINGS ({warnTotal})", ConsoleColor.Cyan);
        if (warnTotal == 0) Line("  (none)");
