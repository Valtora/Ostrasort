...
public sealed class ModEntry
{
    ...
    /// <summary>
    /// The status notes every renderer shows for this mod - ONE list so the GUI
    /// table, console report, text export and markdown export can never drift
    /// apart on what they call things or which facts they include.
    /// </summary>
    public List<string> NoteLines(string? installedVersion)
    {
        var notes = new List<string>();
        if (Ignored) notes.Add("ignored (kept unregistered)");
        else if (!Registered) notes.Add("NOT REGISTERED (not in the game's load list, so it will not load)");
        if (Dir is null && Kind != EntryKind.Core) notes.Add("DEAD ENTRY (its files are missing)");
        if (Disabled) notes.Add("DISABLED (skipped at load)");
        if (IsPatch) notes.Add("generated by Ostrasort");
        ...
        if (LogNotes.Count > 0) notes.Add($"{LogNotes.Count} game-log issue(s) last launch");
        return notes;
    }
}
